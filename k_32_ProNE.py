...
# ProNE (Projection-based Network Embedding)

# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: ProNE – spektralna propagacija + projekcija
 
"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
ProNE (Projection-based Network Embedding): spektralno početno ugrađivanje,
zatim polinomska propagacija (I + μP + μ²P² + …), na kraju projekcija u k dimenzija.
"""
...
PRONE_MU = 0.5  # 0.5 mu for ProNE
PRONE_STEPS = 6 # 6 steps for ProNE
...
def compute_prone_embeddings(M, k=EMBED_DIM):
    ...
    n_comp = min(k, 39)
    U0 = vecs[:, :n_comp].astype(float)
    if U0.shape[1] < k:
        U0 = np.hstack([U0, np.tile(U0[:, -1:], (1, k - U0.shape[1]))])
    Z = U0.copy()
    H = U0.copy()
    for t in range(1, PRONE_STEPS):
        H = P @ H
        Z = Z + (PRONE_MU ** t) * H
...
def main():
    ...
    emb = compute_prone_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (ProNE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (ProNE + kvantna regresija, deterministički, strukturno):
    (6, 8, 23, 26, 32, 35, 39)

    Score: 0.49967099144138377
    """
...
Spektralni početak: 
prve k Laplacian sopstvenih vektora kao U0 (39×k).

Propagacija: 
Z = U0 + μ·(P@U0) + μ²·(P²@U0) + … (PRONE_MU=0.5, 
PRONE_STEPS=6).

Projekcija: SVD od Z, 
uzimanje prvih k levih singularnih vektora (39×k).
Normalizacija: vrednosti u [0, π].
"""
